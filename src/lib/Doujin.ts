...
export function genDoujinEmbed(doujin, tagInfo) {
	...
	if (tagInfo.characters.length != 0)
		mainInfo.addField("Characters:", tagInfo.characters.join(", "), false);
	if (tagInfo.parodies.length != 0)
		mainInfo.addField("Parodies:", tagInfo.parodies.join(", "), false);
	if (tagInfo.languages.length != 0)
		mainInfo.addField("Languages:", tagInfo.languages.join(", "), false);
	if (tagInfo.categories.length != 0)
		mainInfo.addField("Categories:", tagInfo.categories.join(", "), false);
	if (tagInfo.artists.length != 0)
		mainInfo.addField("Artists:", tagInfo.artists.join(", "), false);
	if (tagInfo.groups.length != 0)
		mainInfo.addField("Groups:", tagInfo.groups.join(", "), false);
	mainInfo.addFields(
