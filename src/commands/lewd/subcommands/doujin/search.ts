...
export async function doujinSearch(interaction: ShinanoInteraction) {
    // Fetching & Filtering
    const name: string = interaction.options.getString('doujin-name')
    ...
    const resultNavi: MessageActionRow = new MessageActionRow()
        .addComponents(
    ...
    // Collector
    const message = await interaction.editReply({embeds: [doujinResults[0]], components: [resultNavi]})
    ...
    collector.on('collect', async (i) => {
        ...
        for (let n = 0; n < menu.options.length; n++) {
            (resultNavi.components[0] as MessageSelectMenu).options[n].value === i.values[0]
