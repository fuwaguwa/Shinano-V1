...
export default new Command({
    ...
    run: async({interaction}) => {
        ...
        // Sauce Searching
        wait.setDescription('✅ | Valid Link!\n<a:lod:1021265223707000923> | Searching For Sauce...\n<a:lod:1021265223707000923> | Filtering...')
        await interaction.editReply({embeds: [wait]})
        ...
        sauce.find({url: link}).then(async (sauce) => {
            ...
            if (sauce.results[0].data.source && sauce.results[0].header.index_name.includes('H-Anime')) {
                result.addField('Sauce: ', sauce.results[0].data.source);
                result.addField('Estimated Timestamp: ', sauce.results[0].data.est_time)
            } else {
                if (sauce.results[0].data.member_name) result.addField('Artist: ', sauce.results[0].data.member_name);
                if (sauce.results[0].data.creator) result.addField('Artist: ', `${sauce.results[0].data.creator}`);
                if (sauce.results[0].data.material) result.addField('Material: ', sauce.results[0].data.material);
                if (sauce.results[0].data.characters) result.addField('Character: ', sauce.results[0].data.characters);
                if (sauce.results[0].data.user_name) result.addField('Artist: ', sauce.results[0].data.user_name)
            }
            ...
            if (Object.keys(filteredLink).length == 0) return interaction.editReply({embeds:[result]})
            ...
            if (sauceUrls.components.length === 0) {
                result.setDescription(`Similarity: ${sauce.results[0].header.similarity}%`);
                return interaction.editReply({embeds:[result]})
            }
            result.setFooter({text: 'Similarity is displayed below.'})
            return interaction.editReply({embeds:[result], components: [sauceUrls]})
