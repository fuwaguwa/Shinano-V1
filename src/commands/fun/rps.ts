...
export default new Command({
	...
	run: async ({ interaction }) => {
		...
		if (
			...
			// Buttons
			const AoD = new MessageActionRow().addComponents(
				new MessageButton()
			...
			acceptor.on("end", async (collected, reason) => {
				if (reason !== "ACCEPTED" && reason !== "DECLINED") {
					...
					duel.on("collect", async (i) => {
						...
						const emojiChallengerChoice: string =
							choiceToEmoji(challengerChoice);
						const emojiOpponentChoice: string = choiceToEmoji(opponentChoice);

						// Outputting the winner
						const finalResult: MessageEmbed = new MessageEmbed().setColor(
							"#2f3136"
						);
						...
						for (let i = 0; i < 3; i++) {
							(choices.components[i] as MessageButton)
								.setDisabled(true)
								.setStyle("SECONDARY");
						}
			...
			collector.on("collect", async (i) => {
				...
				for (let i = 0; i < 3; i++) {
					...
					if (
						customId ===
						(choices.components[i] as MessageButton).customId.split("-")[0]
					) {
				...
				const res: MessageEmbed = new MessageEmbed().setColor("#2f3136");
